...
module ServiceHuis {
    ...
    export function loadVerkooppunten(callback: any) {
        $.get("http://cors.sboulema.nl/" + "http://nprverkooppunten.rdw.nl/Productie/verkooppunten.txt", data => {
            ...
            var verkooppunten = new Array<Verkooppunt>();
     ...
     */
    export function loadGebieden(areamanagerid: string, usageId: string, filterOnActive: boolean, callback: any, callbackParams: any) {
        $.getJSON(`https://opendata.rdw.nl/resource/8u4d-s4q7.json?areamanagerid=${areamanagerid}`, data => {
            if (filterOnActive) {
                data = new jinqJs()
                    .from(data)
                    .where<Gebied>(row => (parseDate(row.startdatearea) <= new Date() && parseDate(row.enddatearea) >= new Date()))
                    .select(row => row);
            }
            ...
            if (usageId !== null) {
                data = new jinqJs()
                    .from(data)
                    .where<Gebied>(row => (row.usageid === usageId))
                    .select(row => row);
            }
     ...
     */
    export function loadParkeergebieden(areamanagerid: string, callback: any, callbackParams: any) {
     ...
     */
    export function loadGebiedRegeling(areamanagerid: string, areaid: string, filterOnActive: boolean, callback: any, callbackParams: any) {
        ...
        $.getJSON(url, data => {
            ...
            if (filterOnActive) {
                data = new jinqJs()
                    .from(data)
                    .where<GebiedRegeling>(row => (parseDate(row.startdatearearegulation) <= new Date() && parseDate(row.enddatearearegulation) >= new Date()))
                    .select(row => row);
            }
     ...
     */
    export function loadTijdvak(areamanagerid: string, fareCalculationCode: string, filterOnActive: boolean, callback: any, callbackParams: any) {
        $.getJSON(`https://opendata.rdw.nl/resource/pwnm-2uua.json?areamanagerid=${areamanagerid}`, data => {

            if (filterOnActive) {
                data = new jinqJs()
                    .from(data)
                    .where<Tijdvak>(row => (parseDate(row.startdatetimeframe) <= new Date() && parseDate(row.enddatetimeframe) >= new Date()))
                    .select(row => row);
            }
            ...
            if (fareCalculationCode !== null) {
                data = new jinqJs()
                    .from(data)
                    .where<Tijdvak>(row => (typeof row.farecalculationcode !== "undefined") && row.farecalculationcode.indexOf(fareCalculationCode) > -1)
                    .select(row => row);
            }
     ...
     */
    export function loadTariefdeel(areamanagerid: string, fareCalculationCode: string, filterOnActive: boolean, callback: any, callbackParams: any) {
        ...
        $.getJSON(url, data => {
            ...
            if (filterOnActive) {
                data = new jinqJs()
                    .from(data)
                    .where<Tariefdeel>(row => parseDate(row.startdatefarepart) <= new Date() && parseDate(row.enddatefarepart) >= new Date())
                    .select(row => row);
            }
